package com.dyz.gameserver.msg.processor.wordradio;
...
import com.dyz.gameserver.msg.response.wordradio.WordRadioResponse;
import com.dyz.gameserver.msg.response.wordradio.WordRadioResponseOther;
import com.dyz.gameserver.pojo.Account;
import com.dyz.gameserver.pojo.Message;
import com.dyz.myBatis.services.AccountService;

public class WordRadioChatMsgProcessor extends MsgProcessor implements INotAuthProcessor{

	@Override
	public  void process(GameSession gameSession, ClientRequest request)
			throws Exception {
		String msg=request.getString();
		System.out.println("我收到新的世界广播消息："+msg);
		Message message=new Message();
		message.setChatMsg(msg);
		Avatar avatar=gameSession.getRole(Avatar.class);
		message.setChatFrom(avatar.account);
		gameSession.sendMsg(new WordRadioResponse(1,"true"));
           ...
           if(gameSessionList != null){
               for(int i=0;i<gameSessionList.size();i++){
                   	try {
                   		gameSessionList.get(i).sendMsg(new WordRadioResponseOther(1,message));
                   		System.out.println("我发给收到新的世界广播消息："+msg);
                   	}catch (Exception e){
